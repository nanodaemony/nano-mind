...
使用左外联结。根据题意，**Person 表中的信息一定会出现，所以使用左外联结**。
...
编写一个 SQL 查询，获取 Employee 表中第二高的薪水。
...
将**得分排序，并统计排名**。编写一个 SQL 查询来实现分数排名。如果两个分数相同，则两个分数排名相同。请注意，平分后的下一个名次应该是下一个连续的整数值。换句话说，名次之间不应该有 "间隔"。
...
编写一个 SQL 查询，查找 Person 表中所有重复的电子邮箱。邮件地址表：
...
查找面积超过 `3000000` 或者人口数超过 `25000000` 的国家。
