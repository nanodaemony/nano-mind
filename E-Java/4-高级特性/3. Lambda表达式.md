...
**Function 函数式接口接受一个 T 类型的参数，参数一个 R 类型的结果，用于实现类型转换**。该接口用于将输入的类型 T 转换为输出的类型 R。例如：
...
Predicate 函数式接口接受一个输入参数，返回 boolean 类型的结果，可以用于过滤筛选。Predicate 这个函数式接口像特殊的 `Function<T, Boolean>` 函数式接口。
